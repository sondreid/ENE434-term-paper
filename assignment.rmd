...
table_data <- demand_data_daily %>% filter(date > "2018-07-01") %>% 
...
tibble(Type = "Power demand", 
       Minimum = min(table_data$mWh_demand_daily), 
       Max = max(table_data$mWh_demand_daily),
       "Maximum demand date" = table_data[which.max(table_data$mWh_generated),]$date,
       "Minimum demand date" = table_data[which.min(table_data$mWh_generated),]$date,
       "25% - percentile" = quantile(demand_data_daily$mWh_demand_daily, 0.25),
       "75% - percentile" = quantile(demand_data_daily$mWh_demand_daily, 0.75)) %>% 
  
  bind_rows(
    tibble(Type = "Power generation", 
       Minimum = min(generation_daily$mWh_generated), 
       Max = max(generation_daily$mWh_generated),
       "25% - percentile" = quantile(generation_daily$mWh_generated, 0.25),
       "75% - percentile" = quantile(generation_daily$mWh_generated, 0.75)),
    tibble(Type = "Temperature",
           Minimum  = min(texas_temperature_avg$temp_avg),
           Max      = max(texas_temperature_avg$temp_avg),
           "25% - percentile" = quantile(texas_temperature_avg$temp_avg, 0.25)))  %>% 
  kbl(caption = "Summary statistics of power generaton and power demand in Texas (2015-)") %>%
